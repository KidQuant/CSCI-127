...
# This program takes the location and marks the closets point
...
def getColumnNames():
    ...
    firstColName = input("Enter column name for latitude: ")
    secondColName = input("enter column name for longitude: ")

    latName, lonName = firstColName, secondColName
...
def getLocale():
    """
    Asks the users for latitude and longitude of the user's current location and
    Return those floating points numbers.
    """
    currentLat = input("Enter current latitude: ")
    currentLog = input("enter current longitude: ")

    lat, lon = float(currentLat), float(currentLog)
